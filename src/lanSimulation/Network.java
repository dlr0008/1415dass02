...
<p><strong>Precondition:</strong>this.isInitialized();</p>
	...
	public boolean consistentNetwork () {
		...
		while (iter.hasMoreElements()) {
			...
			if (currentNode.getType() != Node.WORKSTATION) {return false;};
		};
		...
		while (! encountered.containsKey(currentNode.name_)) {
			...
			if (currentNode.getType() == Node.WORKSTATION) {workstationsFound++;};
			if (currentNode.getType() == Node.PRINTER) {printersFound++;};
